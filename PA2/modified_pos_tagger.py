...
class POSTagger():
	...
	def viterbi(self, sentence):
		...
		# print('inital v\n:', v, '\nbacpointer\n:', backpointer)
		for i in range(N):
			v[i, 0] = self.initial[i] + self.emission[sentence[0], i]
		# print('after inital v\n:', v, '\nbacpointer\n:', backpointer)
		# recursion step
		...
		for t in range(1, T):
			for i in range(N):
				# get the max of the t-1-th column of trellis
				max_prev = -np.inf
				for j in range(N):
					max_prev = max(max_prev, v[j, t-1] + self.transition[j, i])
				# add transition weights to each state
				v[i, t] = max_prev + self.emission[sentence[t], i]
		# print('after recursion step 1 v\n:', v, '\nbacpointer\n:', backpointer)
		...
		for t in range(1, T):
			for i in range(N):
				max_prev = -np.inf
				for j in range(N):
					if v[j, t-1] + self.transition[j, i] > max_prev:
						max_prev = v[j, t-1] + self.transition[j, i]
						backpointer[i, t] = j
		# print('after recursion step 2 v\n:', v, '\nbacpointer\n:', backpointer)
		# termination step
...
def train(self, train_set, dummy_data=None):
    ...
    for i, sentence_id in enumerate(sentence_ids):
        ...
        # Debug: Print the current sentence and its tags
