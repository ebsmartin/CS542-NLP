...
expected_word_set = ['Alice', 'Dorothy', 'admired', 'cheered', 'dwarf', 'every']
...
sentence_ids, sentences, tag_lists, word_lists = pos.load_data('data_small/train')

split_sentences = {}
for s in sentence_ids:
    split_sentences[s] = [word_tag.rsplit('/', maxsplit=1) for word_tag in sentences[s].split()]

output_tag_lists = [[pos.tag_dict[word_tag[1]] for word_tag in split_sentences[s]] for s in sentence_ids]
output_word_lists = [[pos.word_dict[word_tag[0]] for word_tag in split_sentences[s]] for s in sentence_ids]
...
sentences = {'ca020': 'Alice/nn cheered/vb', 'ca050': 'Goldilocks/nn cheered/vb'}
tag_lists = {'ca020': [0, 1], 'ca050': [0, 1]}
word_lists = {'ca020': [0, 5], 'ca050': [-1, 5]}

sentences, results = pos.test('data_small/test',dummy_data=(sentence_ids,sentences,tag_lists,word_lists))
