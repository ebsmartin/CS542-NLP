...
class POSTagger():
	...
	# 		for s in range(N):
	# 			trans_probs = [v[s_prime, t-1] + self.transition[s_prime, s] for s_prime in range(N)]
	...
	def train(self, train_set, dummy_data=None):
		...
		for i, sentence_id in enumerate(sentence_ids):
			...
			# Debugging stuff...
			# # if the lengths are different, something went wrong; skip this iteration
			# if len(words) != len(predicted_tags):
			# 	print(f"Warning: Mismatch in tags length for sentence_id {sentence_id}. Words: {len(words)}, Predicted Tags: {len(predicted_tags)}")
			# 	continue
			...
			if predicted_tags != correct_tags:
				...
				print("up", correct_tags[0], "down", predicted_tags[0])
				...
				for t, word in enumerate(words):
					word_idx = self.word_dict.get(word, self.unk_index)
					correct_tag_idx = self.tag_dict.get(correct_tags[t], -1)
					predicted_tag_idx = self.tag_dict.get(predicted_tags[t], -1)

					# Update only if the tag index exists (I did this because I messed up but I will keep it)
					if correct_tag_idx != -1:
						self.emission[word_idx, correct_tag_idx] += 1  # update emission weight pos

					if predicted_tag_idx != -1:
						self.emission[word_idx, predicted_tag_idx] -= 1  # update emission weights neg
	...
	def test(self, dev_set, dummy_data=None):
		...
		for i, sentence_id in enumerate(sentence_ids):
			...
			# Ensure that the predicted tags list length matches the correct tags list length
			if len(predicted_tags) != len(correct_tags):
				print(f"Warning: Mismatch in tags length for sentence_id {sentence_id}. Correct: {len(correct_tags)}, Predicted: {len(predicted_tags)}")
